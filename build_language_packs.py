#!/usr/bin/env python3
"""Build EasyDiscuss language pack ZIPs — one per locale, with Joomla XML manifest."""
...
\t<name>EasyDiscuss - Language Pack {name} ({locale})</name>
...
\t<description>{name} Language Pack for EasyDiscuss {VERSION}</description>
\t<fileset>
...
locales = sorted([
    ...
    if d.is_dir()
    and d.name != 'en-GB'
    and re.match(r'^[a-z]{2}(-[A-Z]{2}|Cyrl)$', d.name)
])
...
for locale in locales:
    ...
    manifest_name = f'easydiscuss_{locale}.xml'

    zip_path = DIST / f'com_easydiscuss_{locale}.zip'
